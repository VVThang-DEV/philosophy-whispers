...
void main() {
    ...
    // Sample with blur for unfocused items
    vec4 color = vec4(0.0);
    float blurAmount = (1.0 - vAlpha) * 0.003 * (1.0 - uFocusFactor);
    ...
    if (blurAmount > 0.0001) {
        ...
        for(float x = -2.0; x <= 2.0; x += 1.0) {
            for(float y = -2.0; y <= 2.0; y += 1.0) {
                ...
                color += texture(uTex, st + offset);
            }
        }
        color /= 25.0;
    } else {
        color = texture(uTex, st);
    }
    
    // Apply focus factor
    float focusEffect = mix(0.4, 1.0, vAlpha * (0.6 + uFocusFactor * 0.4));
    color.rgb *= focusEffect;
    color.a *= vAlpha;
...
class InfiniteGridMenu {
  ...
  init(onInit?: (menu: InfiniteGridMenu) => void) {
    ...
    this.discLocations = {
      ...
      uFocusFactor: gl.getUniformLocation(this.discProgram, "uFocusFactor"),
    };
  ...
  initTexture() {
    ...
    const loadImage = (src: string) => {
      const tryCreateImage = (url: string) =>
      ...
      const runner = async () => {
        // First try direct <img> load
        let img = await tryCreateImage(src);
        if (img) return img;

        // Fallback: try fetch as blob and createObjectURL
        try {
          const resp = await fetch(src, { mode: "cors" });
          if (!resp.ok) throw new Error("fetch failed");
          const blob = await resp.blob();
          const objectUrl = URL.createObjectURL(blob);
          img = await tryCreateImage(objectUrl);
          if (img) {
    ...
    Promise.all(this.items.map((it) => loadImage(it.image))).then((images) => {
      // Draw each image into atlas with aspect-fit. If an image failed, draw placeholder.
      images.forEach((img, i) => {
        ...
        if (!img) {
          ...
          g.addColorStop(0, "#4c2f7a");
          g.addColorStop(1, "#14121a");
          ctx.fillStyle = g;
          ctx.beginPath();
        ...
        if (imgAspect > targetAspect) {
          // image is wider
          dw = Math.round(targetSize);
          dh = Math.round(targetSize / imgAspect);
          dx = x;
          dy = y + Math.round((targetSize - dh) / 2);
        } else {
          // image is taller or equal
          dw = Math.round(targetSize * imgAspect);
          dh = Math.round(targetSize);
          dx = x + Math.round((targetSize - dw) / 2);
          dy = y;
        }
      ...
      // Upload atlas to GL texture. Use ALIGNMENT and NPOT-safe params.
      gl.bindTexture(gl.TEXTURE_2D, this.tex);
      // Ensure pixel storage alignment
      ...
      // For NPOT (non-power-of-two) textures, set wrapping to CLAMP_TO_EDGE and no mipmaps
      const isPOT = (n: number) => (n & (n - 1)) === 0;
      if (isPOT(canvas.width) && isPOT(canvas.height)) {
        gl.generateMipmap(gl.TEXTURE_2D);
      } else {
        // If atlas is NPOT, ensure min filter is linear and no mipmaps used
      ...
      // Dev debugging: log atlas info
      try {
        console.info("SphereGallery: uploaded atlas", {
  ...
  render() {
    ...
    // Focus factor: 1.0 when not moving, 0.0 when moving
    const focusFactor = this.movementActive ? 0.0 : 1.0;
    gl.uniform1f(this.discLocations.uFocusFactor, focusFactor);
...
export default function DomeGallery({
  ...
  const [activePhilosopher, setActivePhilosopher] =
  ...
  useEffect(() => {
    ...
    const items = philosophers.map((p) => ({
      image: p.avatar,
      id: p.id,
    }));

    const handleActiveItem = (index: number) => {
      const philosopher = philosophers[index % philosophers.length];
      setActivePhilosopher(philosopher);
    };
  ...
  const handlePhilosopherClick = () => {
    if (activePhilosopher && onPhilosopherSelect) {
      onPhilosopherSelect(activePhilosopher);
    }
  };
    ...
    <div className="relative w-full h-full bg-transparent">
      ...
      {activePhilosopher && (
        <>
          <div
            className={`
            absolute left-8 top-1/2 -translate-y-1/2 max-w-md
          ...
          `}
          >
            <h2 className="text-5xl md:text-6xl font-black text-[hsl(40,20%,95%)] mb-4 drop-shadow-[0_4px_30px_rgba(0,0,0,0.9)]">
              {activePhilosopher.name}
            </h2>
            <p className="text-lg text-[hsl(270,60%,75%)] font-medium drop-shadow-[0_0_10px_hsl(270,60%,50%)]">
              {activePhilosopher.school}
            </p>
            <p className="text-sm text-[hsl(270,60%,80%)] mt-1 drop-shadow-[0_0_8px_hsl(270,60%,50%)]">
              {activePhilosopher.era}
            </p>
          </div>

          <div
            className={`
            absolute right-8 top-1/2 -translate-y-1/2 max-w-xs
            transition-all duration-500 ease-out
            ${
              isMoving
                ? "opacity-0 pointer-events-none translate-x-[50px]"
          ...
          `}
          >
            <p className="text-[hsl(40,20%,95%)]/90 text-base leading-relaxed italic drop-shadow-[0_2px_20px_rgba(0,0,0,0.8)]">
              "{activePhilosopher.famousQuote}"
            </p>
          </div>
          ...
          <button
            ...
            bg-gradient-to-r from-[hsl(220,70%,55%)] via-[hsl(270,60%,50%)] to-[hsl(270,60%,60%)]
            text-white font-bold text-lg
            shadow-[0_0_40px_hsl(270,60%,50%,0.5)]
            hover:scale-110 hover:shadow-[0_0_60px_hsl(270,60%,50%,0.7)]
            active:scale-95
          ...
          `}
          >
            Trò chuyện với {activePhilosopher.name.split(" ")[0]} →
          </button>
        </>
      )}
